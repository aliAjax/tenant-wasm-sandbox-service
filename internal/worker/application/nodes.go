...
func (r *NodeRegistry) Drain(ctx context.Context, id string) (worker.Node, error) {
	...
	if v.State == worker.NodeIsolated || v.State == worker.NodeStopped {
...
func (r *NodeRegistry) Ready() bool {
	r.mu.RLock()
	defer r.mu.RUnlock()
	return r.nodes[r.localID].State == worker.NodeReady
}
