...
func (s *Simulator) Execute(ctx context.Context, module runtime.CompiledModule, entry string, in execution.Invocation, budget execution.Budget) (runtime.Result, error) {
	...
	m, ok := module.(compiled)
	if !ok {
		return runtime.Result{}, fmt.Errorf("foreign compiled module")
	}
	if entry == "" {
	...
	return runtime.Result{Output: out, ExitCode: p.ExitCode, Truncated: truncated, Usage: execution.Usage{Instructions: p.InstructionCost, MemoryPages: p.MemoryPages, CPUTime: time.Since(started), OutputBytes: 0}}, nil
}
...
func (s *Simulator) Close(context.Context) error { s.closed.Store(true); return nil }
