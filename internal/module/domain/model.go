...
func New(id string, in RegisterRequest, maxBytes int64, now time.Time) (Module, error) {
	...
	return Module{ID: id, TenantID: in.TenantID, Name: in.Name, Version: in.Version, Digest: actual, Entry: in.Entry, ABI: in.ABI,
		Runtime: in.Runtime, Capabilities: append([]Capability(nil), in.Capabilities...), EnvAllowlist: append([]string(nil), in.EnvAllowlist...),
		ImportList: append([]string(nil), in.ImportList...), Size: int64(len(in.Content)), ObjectKey: in.ObjectKey, State: StateDraft, CreatedAt: now.UTC()}, nil
}
